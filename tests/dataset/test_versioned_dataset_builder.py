...
def _create_test_dataset_local() -> None:
    ...
    for filename in TEST_DATASET_FILENAMES:
        dirname = os.path.dirname(
            os.path.join(TEST_DATASET_PATH_LOCAL, filename)
        )
...
def test_publish_local_path_creates_expected_files() -> None:
    ...
    assert set(os.listdir(publication_dir)) == expected_features.union(
        expected_labels
    ).union({"data_processor.pkl", "meta.json", "raw.tar.bz2"})
...
def test_publish_s3_path_creates_expected_files(mocked_s3: None) -> None:
    ...
    expected_features = {"X_train.npy", "X_val.npy", "X_test.npy"}
    expected_labels = {"y_train.npy", "y_val.npy", "y_test.npy"}
    expected_keys = expected_features.union(expected_labels).union(
...
def test_publish_from_raw_dataset_in_s3_to_local(mocked_s3: None) -> None:
    ...
    for current_path, _, filenames in os.walk(raw_dataset_dir):
        for filename in filenames:
            raw_dataset_paths.add(
                os.path.join(current_path, filename).replace(
                    raw_dataset_dir + "/", "", 1
                )
            )
...
def test_publish_from_raw_dataset_in_s3_to_s3(mocked_s3: None) -> None:
    ...
    dataset_filenames = {
        os.path.join(raw_dataset_dir, name) for name in TEST_DATASET_FILENAMES
    }
...
def test_publish_zips_raw_dataset() -> None:
    ...
    raw_dataset_zip = os.path.join(
        TEST_PUBLICATION_PATH_LOCAL, version, "raw.tar.bz2"
    )
...
def test_publish_zips_s3_to_s3(mocked_s3: None) -> None:
    ...
    raw_dataset_zip = os.path.join(
        TEST_PUBLICATION_PATH_S3, version, "raw.tar.bz2"
    )
    fs = S3FileSystem()
...
def test_publish_includes_raw_dataset_link() -> None:
    ...
    with open(
        os.path.join(raw_dataset_dir, link_filename), "r", encoding="utf-8"
    ) as infile:
...
def test_publish_includes_raw_dataset_link_s3(mocked_s3: None) -> None:
    ...
    assert set(fs.ls(raw_dataset_dir)) == {
        os.path.join(raw_dataset_dir, link_filename)[5:]
    }
    with fs.open(
        os.path.join(raw_dataset_dir, link_filename), "r", encoding="utf-8"
    ) as infile:
...
def test_same_datasets_have_same_hashes() -> None:
    ...
    meta_path1 = os.path.join(TEST_PUBLICATION_PATH_LOCAL, "v1", "meta.json")
    meta_path2 = os.path.join(TEST_PUBLICATION_PATH_LOCAL, "v2", "meta.json")
...
def test_rebuilt_datasets_have_same_hashes_local_to_s3(
    ...
    meta_path1 = os.path.join(TEST_PUBLICATION_PATH_S3, "v1", "meta.json")
    meta_path2 = os.path.join(TEST_PUBLICATION_PATH_S3, "v2", "meta.json")
    fs = S3FileSystem()
...
def test_publish_local_and_s3_create_same_dataset(mocked_s3: None) -> None:
    ...
    meta_path1 = os.path.join(
        TEST_PUBLICATION_PATH_LOCAL, version, "meta.json"
    )
    with open(meta_path1, "r", encoding="utf-8") as infile:
        contents1 = json.loads(infile.read())
    s3 = boto3.client("s3")
    ...
    # Remove leading slash
    prefix = parse_result.path[1:]
    contents2 = s3.get_object(
        Bucket=bucket_name, Key=os.path.join(prefix, version, "meta.json")
    )
...
def test_published_data_processor_reproduces_dataset() -> None:
    ...
    processor_filename = os.path.join(
        TEST_PUBLICATION_PATH_LOCAL, version, "data_processor.pkl"
    )
