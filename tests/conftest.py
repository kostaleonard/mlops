...
from tests.dataset.test_versioned_dataset import (
...
def fixture_dataset() -> VersionedDataset:
    ...
    return VersionedDataset(
        os.path.join(TEST_DATASET_PUBLICATION_PATH_LOCAL, "v1")
    )
...
def fixture_dataset_s3(mocked_s3: None) -> VersionedDataset:
    ...
    _publish_test_dataset_s3()
    return VersionedDataset(
        os.path.join(TEST_DATASET_PUBLICATION_PATH_S3, "v1")
    )
...
def fixture_model(dataset: VersionedDataset) -> Model:
    ...
    mod = Sequential(
        [
            Dense(
                dataset.y_train.shape[1], input_shape=dataset.X_train.shape[1:]
            )
        ]
    )
...
def fixture_training_config(
    ...
    """
    train_kwargs = {"epochs": 5, "batch_size": 8}
    history = model.fit(x=dataset.X_train, y=dataset.y_train, **train_kwargs)
