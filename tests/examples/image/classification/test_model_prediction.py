...
from tests.examples.image.classification.test_train_model import (
    _create_dataset,
    TEST_DATASET_PUBLICATION_PATH_LOCAL,
)
...
def _create_model() -> str:
    ...
    training_config = train_model(model, dataset, **train_kwargs)
...
def test_get_best_model_gets_lowest_val_error() -> None:
    ...
    for fake_train_history in FAKE_TRAIN_HISTORIES:
        ...
        with open(
            os.path.join(publication_path, "meta.json"), "r", encoding="utf-8"
        ) as infile:
            contents = json.loads(infile.read())
        contents["history"] = fake_train_history
        with open(
            os.path.join(publication_path, "meta.json"), "w", encoding="utf-8"
        ) as outfile:
